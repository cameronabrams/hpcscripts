...
help () {
    ...
    echo "    --tarball                      make a tarball with all necessary inputs for next run"
    echo "    -q                             suppress messages"
...
if [ -z "${lastout}" ]; then
    lastout=`grep -i ^outputname $CONF | tail -1 | awk '{print $2}' | sed 's/;$//'`
    if [ -z "${lastout}" ]; then
...
if [[ $tarball == "1" ]]; then
    tar zvcf ${RECONF}.tgz `cat ${RECONF}.files`
    echo "Created tarball ${RECONF}.tgz"
fi
